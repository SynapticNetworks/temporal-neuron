...
type ChannelTrigger struct {
	...
	// === LIGAND DEPENDENCE ===
	LigandType          message.LigandType `json:"ligand_type"`          // Required neurotransmitter
...
type IonChannel interface {
	...
	//
	// Returns:
	//   *message.NeuralSignal: The modified signal (or nil if blocked)
	//   bool: Whether the signal should continue processing
	//   float64: Additional ionic current generated by this channel (pA)
	ModulateCurrent(msg message.NeuralSignal, voltage, calcium float64) (*message.NeuralSignal, bool, float64)
...
type ChannelConfig struct {
	...
	// === LIGAND DEPENDENCE ===
	RequiredLigand    message.LigandType `json:"required_ligand"`
...
type DendriticIntegrationMode interface {
	...
	Handle(msg message.NeuralSignal) *IntegratedPotential
...
func (m *PassiveMembraneMode) Handle(msg message.NeuralSignal) *IntegratedPotential {
...
func NewTemporalSummationMode() *TemporalSummationMode {
	return &TemporalSummationMode{
		buffer:       make([]message.NeuralSignal, 0, DEFAULT_INPUT_BUFFER_SIZE),
		channelChain: make([]IonChannel, 0),
	}
}

// Handle processes the message through ion channels and buffers for integration.
func (m *TemporalSummationMode) Handle(msg message.NeuralSignal) *IntegratedPotential {
...
func (m *TemporalSummationMode) Process(state MembraneSnapshot) *IntegratedPotential {
	...
	currentBatch := make([]message.NeuralSignal, len(m.buffer))
...
func (m *BiologicalTemporalSummationMode) Handle(msg message.NeuralSignal) *IntegratedPotential {
	now := time.Now()
...
func (m *ActiveDendriteMode) Handle(msg message.NeuralSignal) *IntegratedPotential {
	now := time.Now()
