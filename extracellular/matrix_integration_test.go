...
func (mn *MockNeuron) Bind(ligandType LigandType, sourceID string, concentration float64) {
	...
	case LigandGlutamate:
		mn.currentPotential += concentration * 0.5 // Excitatory
	case LigandGABA:
		mn.currentPotential -= concentration * 0.3 // Inhibitory
	case LigandDopamine:
		mn.currentPotential += concentration * 0.2 // Modulatory
	}

	// Check if neuron should fire
	if mn.currentPotential > mn.firingThreshold && mn.isActive {
		// Neuron fires! (would trigger action potential in real system)
		mn.currentPotential = 0.0 // Reset after firing
	}
}
...
// Implement SignalListener interface (for electrical signaling)
func (mn *MockNeuron) OnSignal(signalType SignalType, sourceID string, data interface{}) {
	switch signalType {
	case SignalFired:
		// Respond to action potential from connected neuron
		if value, ok := data.(float64); ok && mn.isActive {
			mn.currentPotential += value * 0.1 // Electrical coupling effect
		}
