...
func TestMatrixEdgeResourceLimits(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			matrix.RegisterNeuronType("edge_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
				return NewMockNeuron(id, config.Position, config.Receptors), nil
			})

			matrix.RegisterSynapseType("edge_synapse", func(id string, config SynapseConfig, callbacks SynapseCallbacks) (SynapseInterface, error) {
			...
			var createdNeurons []NeuronInterface
			neuronErrors := 0
			...
			if len(createdNeurons) >= 2 && tc.synapsesToTry > 0 {
				...
				var createdSynapses []SynapseInterface
				synapseErrors := 0
...
func TestMatrixEdgeConcurrencyStress(t *testing.T) {
	...
	matrix.RegisterNeuronType("stress_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
		return NewMockNeuron(id, config.Position, config.Receptors), nil
	})

	matrix.RegisterSynapseType("stress_synapse", func(id string, config SynapseConfig, callbacks SynapseCallbacks) (SynapseInterface, error) {
		return NewMockSynapse(id, config.Position, config.PresynapticID, config.PostsynapticID, config.InitialWeight), nil
	})
	...
	for _, neuron := range actualNeurons[:10] { // Limit to avoid overwhelming
		matrix.ListenForSignals([]SignalType{SignalFired, SignalConnected}, neuron)
	}
...
func TestMatrixEdgeStateTransitions(t *testing.T) {
	...
	for i := 0; i < CYCLE_COUNT; i++ {
		...
		matrix.RegisterNeuronType("cycle_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
			return NewMockNeuron(id, config.Position, config.Receptors), nil
		})
	...
	// Register factory
	matrix.RegisterNeuronType("shutdown_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
		return NewMockNeuron(id, config.Position, config.Receptors), nil
	})
...
func TestMatrixEdgeBiologicalConstraints(t *testing.T) {
	...
	matrix.RegisterNeuronType("constraint_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
		return NewMockNeuron(id, config.Position, config.Receptors), nil
	})

	testNeuron, err := matrix.CreateNeuron(NeuronConfig{
	...
	neuron2, err := matrix.CreateNeuron(NeuronConfig{
		NeuronType: "constraint_neuron",
	...
	if neuron2 != nil {
		...
		matrix.RegisterSynapseType("constraint_synapse", func(id string, config SynapseConfig, callbacks SynapseCallbacks) (SynapseInterface, error) {
			return NewMockSynapse(id, config.Position, config.PresynapticID, config.PostsynapticID, config.InitialWeight), nil
		})
...
func TestMatrixEdgeMemoryManagement(t *testing.T) {
	...
	for cycle := 0; cycle < LIFECYCLE_CYCLES; cycle++ {
		...
		// Register factory
		matrix.RegisterNeuronType("memory_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
			return NewMockNeuron(id, config.Position, config.Receptors), nil
		})
	...
	// Register factory
	sustainedMatrix.RegisterNeuronType("sustained_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
		return NewMockNeuron(id, config.Position, config.Receptors), nil
	})

	// Create initial population
	var sustainedNeurons []NeuronInterface
	for i := 0; i < 10; i++ {
		config := NeuronConfig{
...
func TestMatrixEdgePerformanceDegradation(t *testing.T) {
	...
	// Register factories
	matrix.RegisterNeuronType("perf_neuron", func(id string, config NeuronConfig, callbacks NeuronCallbacks) (NeuronInterface, error) {
		return NewMockNeuron(id, config.Position, config.Receptors), nil
	})

	matrix.RegisterSynapseType("perf_synapse", func(id string, config SynapseConfig, callbacks SynapseCallbacks) (SynapseInterface, error) {
		return NewMockSynapse(id, config.Position, config.PresynapticID, config.PostsynapticID, config.InitialWeight), nil
	})
...
func TestActualBiologicalFactories(t *testing.T) {
	...
	_, err := matrix.CreateNeuron(NeuronConfig{
