...
func TestAstrocyteNetworkEdgeCaseStress(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		network.Register(ComponentInfo{
			...
			Type:     ComponentNeuron,
			Position: types.Position3D{X: float64(i), Y: float64(i), Z: 0},
			State:    StateActive,
		})
	}
	...
	for i := 0; i < lifecycleIterations; i++ {
		...
		err := network.Register(ComponentInfo{
			ID:       componentID,
			Type:     ComponentNeuron,
			Position: types.Position3D{X: float64(i), Y: 0, Z: 0},
			State:    StateActive,
		})
		if err != nil {
			lifecycleErrors++
	...
	for i := 0; i < memoryTestCount; i++ {
		...
		err := network.Register(ComponentInfo{
			...
			Type:     ComponentNeuron,
			Position: types.Position3D{X: float64(i), Y: float64(i), Z: float64(i)},
			State:    StateActive,
			Metadata: metadata,
		})
